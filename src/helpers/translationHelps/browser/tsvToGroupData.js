...
export function convertReferenceChunksToString(chunks) {
  ...
  try {
    ...
    console.warn(
      `convertReferenceChunksToString() - invalid chunks: "${JSON.stringify(
        chunks
      )}"`,
      e
    )
...
//  */
export function tnJsonToGroupData(
  originalBible,
  bookId,
  tsvObjects,
  resourcesPath,
  ...
  class ManageResource {
    constructor(originalBible, bookId) {
      this.originalBible = originalBible
      this.bookId = bookId
    }

    getVerseStringFromRef(ref) {
      const chunks = cleanupReference(ref)
      const refStr = convertReferenceChunksToString([chunks])
      const referenceList = parseReferenceToList(refStr)
      return originalBible[referenceList[0].chapter][referenceList[0].verse]
    }
  }

  try {
    const resourceApi = new ManageResource(originalBible, bookId.toLowerCase())
    ...
    for (const tsvItem of tsvObjects) {
      if (tsvItem.SupportReference && tsvItem.OrigQuote) {
        ...
        tsvItem.OccurrenceNote = cleanOccurrenceNoteLinks(
          tsvItem.OccurrenceNote || '',
          resourcesPath,
          langId,
          bookId.toLowerCase(),
          tsvItem.Chapter
        ...
        if (!tsvItem.OccurrenceNote) {
          console.warn(
            'tsvToGroupData() - error processing item:',
            JSON.stringify(tsvItem)
          )
          continue
        }
        ...
        try {
          verseString = resourceApi.getVerseStringFromRef(
            tsvItem.Reference || `${tsvItem.Chapter}:${tsvItem.Verse}`
          )
        } catch (e) {
          if (parseInt(tsvItem.Chapter, 10) && parseInt(tsvItem.Verse, 10)) {
            // only if chapter and verse are valid do we expect verse text
            console.warn(
              `tsvToGroupData() - error getting verse string: chapter ${
                tsvItem.Chapter
              }, verse ${tsvItem.Verse} from ${JSON.stringify(tsvItem)}`,
              e
            )
          }
        }
        ...
        if (verseString) {
          const groupDataItem = generateGroupDataItem(
            tsvItem,
            toolName,
            verseString
          )
    ...
    console.log({ groupData: categorizeGroupData(groupData) })
    return params && params.categorized
      ? categorizeGroupData(groupData)
      : groupData
  } catch (e) {
    console.error(
      `tsvToGroupData() - error processing filepath: ${filepath}`,
      e
    )
    throw e
  }
...
export const tsvToGroupData = async (
  filepath,
  toolName,
  params = {},
  originalBiblePath,
...
//  */
// // eslint-disable-next-line require-await
export async function tsvToObjects(tsvString) {
  const tsvLines = tsvString
  ...
  if (tableObject.errors.length > 0) {
    ...
    console.warn(
      `twArticleHelpers.twlTsvToGroupData() - table parse errors found: ${parseErrorMsg}`
    )
  }
  ...
  try {
    ...
    console.error(
      `twArticleHelpers.twlTsvToGroupData() - error processing filepath: ${tsvPath}`,
      e
    )
    error = e
  }
...
//  */
export const tsvToGroupData7Cols = async (
  tsvString,
  bookId,
  resourcesPath,
  langId,
  ...
  const { tsvItems, error } = await tsvToObjects(tsvString, {})
  ...
  try {
    const groupData = tnJsonToGroupData(
      originalBible,
      bookId,
      tsvObjects,
      resourcesPath,
    ...
    console.error(
      `tsvToGroupData7Cols() - error processing filepath: ${filepath}`,
      e
    )
    throw e
  }
 ...
 * @param {string} langId
 */
export const convertLinkToMarkdownLink = (
  tHelpsLink,
  resourcesPath,
  langId
) => {
  try {
    ...
    if (resource === 'tw') {
      articlePath =
        path.join(resourcePath, category.split('/')[1], 'articles', file) +
        '.md'
    }
    ...
    console.error(
      `convertLinkToMarkdownLink() - invalid link: ${tHelpsLink}, resourcesPath: ${resourcesPath}, langId: ${langId}`,
      e
    )
    return null
  }
...
export const fixBibleLink = (link, resourcesPath, langId, bookId, chapter) => {
  const bibleId = getBibleIdForLanguage(
    path.join(resourcesPath, langId, 'bibles')
  )
  ...
  return (
    parts[1] +
    '(rc://' +
    [langId, bibleId, 'book', linkBookId, linkChapter, parts[7]].join('/') +
    ')'
  )
}
 ...
 */
export const cleanOccurrenceNoteLinks = (
  occurrenceNote,
  resourcesPath,
  langId,
  bookId,
  ...
  try {
    ...
    const rightParenBetweenLinksPattern =
      /(?<=\[\[rc:\/\/[^\]]+]])\)(?=[^\(]+rc:)/g
    ...
    if (resourcesPath && langId) {
      ...
      cleanNote = cleanNote.replace(tHelpsPattern, link => {
        let convertedLink = convertLinkToMarkdownLink(
          link,
          resourcesPath,
          langId
        )
        ...
        if (!convertedLink) {
          throw new Error(
            `cleanOccurrenceNoteLinks() - error converting link: ${link}`
          )
        }
    ...
    if (bookId && langId && resourcesPath) {
      ...
      cleanNote = cleanNote.replace(bibleLinkPattern, link =>
        fixBibleLink(link, resourcesPath, langId, bookId, chapter)
      )
    }
    return cleanNote
  } catch (e) {
    console.error(
      `cleanOccurrenceNoteLinks() - occurrenceNote: ${occurrenceNote}, resourcesPath: ${resourcesPath}, langId: ${langId}, bookId: ${bookId}, chapter: ${chapter}`,
      e
    )
    return null
  }
...
export const generateGroupDataItem = (tsvItem, toolName, verseString) => {
  ...
  const wordOccurrencesForQuote = getWordOccurrencesForQuote(
    OrigQuote,
    verseString,
    true
  ) // uses tokenizer to get list of words handle various punctuation and spacing chars
  const quote =
    wordOccurrencesForQuote.length > 1 || hasEllipsis(OrigQuote)
      ? wordOccurrencesForQuote
      : OrigQuote // only use array if more than one word found
